...
run:
LMCF_ROOT=/path/to/dir python train.py python train.py
...
# ── Run selection ─────────────────────────────────────────────────────────────
# comment out M and E which is not needed
MODELS_TO_RUN = [
    'M1',
    'M2',
    'M3',
...
# TinyStories Dataset is huge.
# Used stream technique to load a part of dataset consecutively
...
# load dataset from dir if exits else download
...
# ═══════════════════════════════════════════════════════════════════════════════
# BPE TOKENIZER (trained from scratch one time and save)
# ═══════════════════════════════════════════════════════════════════════════════
# Load from dir if exist else train from scratch 
...
def run_experiment(model_name='M1', exp_key='E1', max_steps=None,
    ...
    try:
        ppl_a_val = s_ppl_a
        ppl_b_val = s_ppl_b
        ppl_a_tst = s_ppl_a_tst
        ppl_b_tst = s_ppl_b_tst
    except NameError:
...
# ═══════════════════════════════════════════════════════════════════════════════
# GENERATION EVAL + CSV  (two rows per run — one per stage)
# ═══════════════════════════════════════════════════════════════════════════════
...
def compute_generation_metrics(samples: list) -> dict:
    ...
    return {
        ...
        'rep_rate': round(sum(s['rep_rate'] for s in samples) / max(1, len(samples)), 4),
...
for run_id, res in results.items():
    ...
    for s_idx, hist in enumerate(hists):
        ...
        # Loss (left axis)
        ax_loss.plot(xs, hist['train_loss'], color=c, ls='-',  ms=3, marker='o', label=f'train S{s_idx+1}')
        ax_loss.plot(xs, hist['val_loss'],   color=c, ls='--', ms=3, marker='s', label=f'val S{s_idx+1}')

        # LR (right axis) — gray dotted, one line per stage
        if hist.get('lr'):
            ax_lr.plot(xs, hist['lr'], color='gray', ls=':', lw=1.5, alpha=0.8, label=f'LR S{s_idx+1}')

        # PPL
        ax_ppl.plot(xs, hist['train_ppl'], color=c, ls='-',  ms=3, marker='o', label=f'train S{s_idx+1}')
        ax_ppl.plot(xs, hist['val_ppl'],   color=c, ls='--', ms=3, marker='s', label=f'val S{s_idx+1}')
        ...
        if ax_fgt and forget_domain and learn_domain and s_idx == 1:
            ...
            if hist.get('val_ppl'):
                ax_fgt.plot(xs, hist['val_ppl'], 'tab:blue', ls='-', lw=2, label=f'{learn_domain}-val S2 (learning ↓)')
            # Forgetting domain PPL going UP
            if ep.get(forget_domain):
                ax_fgt.plot(xs, ep[forget_domain], 'tab:red', ls='--', lw=2, label=f'{forget_domain}-val S2 (forgetting ↑)')

            if s_idx == len(hists) - 1:
                ax_fgt.set(title=f'{run_id} — Forgetting Signal', xlabel='Step', ylabel='PPL')
    ...
    ax_loss.set(title=f'{run_id} — Loss + LR', xlabel='Step', ylabel='Cross-entropy loss')
